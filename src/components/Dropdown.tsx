...
export default function Dropdown({
  ...
  return (
    <div className={cn("relative", className)} ref={dropdownRef} id={id}>
      ...
      {isOpen && (
        <div className="absolute z-50 mt-1 max-h-60 w-full overflow-auto rounded-xl border border-white/10 bg-slate-900 py-1 shadow-xl shadow-black/50 animate-in fade-in zoom-in-95 focus:outline-none">
          {options.map((option) => (
            <button
              key={option.value}
              type="button"
