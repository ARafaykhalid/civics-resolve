...
import { announcementSchema, volunteerSchema, eventSchema } from "@/lib/validations";

// ============ ANNOUNCEMENTS ============

export async function createAnnouncement(data: { title: string; content: string; type: string; isPinned: boolean }) {
  try {
    ...
    await Announcement.create({ ...validated, isActive: true, createdBy: session.user.id });
    revalidatePath("/");
...
export async function getAnnouncements(activeOnly = true) {
  try {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    return { success: true, data: announcements.map((a: any) => ({
      ...a, _id: a._id.toString(), createdBy: a.createdBy?.toString(),
...
export async function getVolunteerOpportunities(activeOnly = true) {
  try {
    await connectDB();
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const query: any = activeOnly ? { isActive: true, date: { $gte: new Date() } } : {};
    const volunteers = await Volunteer.find(query).sort({ date: 1 }).lean();
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    return { success: true, data: volunteers.map((v: any) => ({
      ...v, _id: v._id.toString(), createdBy: v.createdBy?.toString(),
      volunteers: v.volunteers?.map((id: mongoose.Types.ObjectId) => id.toString()) || [],
...
export async function createEvent(data: {
  ...
  date: string; endDate?: string; image?: string; organizer: string; maxAttendees?: number;
}) {
  try {
    ...
    if (!session?.user) return { success: false, error: "Auth required" };
    const role = (session.user as { role: string }).role;
    if (!["admin", "ngo", "authority"].includes(role)) return { success: false, error: "Insufficient permissions" };
...
export async function getEvents(activeOnly = true) {
  try {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    return { success: true, data: events.map((e: any) => ({
      ...e, _id: e._id.toString(), createdBy: e.createdBy?.toString(),
      attendees: e.attendees?.map((id: mongoose.Types.ObjectId) => id.toString()) || [],
...
export async function joinEvent(eventId: string) {
  try {
    ...
    if (event.maxAttendees && event.attendees.length >= event.maxAttendees) return { success: false, error: "Event full" };

    const userId = new mongoose.Types.ObjectId(session.user.id);
    if (event.attendees.some((a) => a.equals(userId))) return { success: false, error: "Already joined" };
