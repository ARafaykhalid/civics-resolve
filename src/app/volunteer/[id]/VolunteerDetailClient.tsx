...
export default function VolunteerDetailClient({
  ...
  const hasJoined = session?.user?.id
    ? v.volunteers?.some((vol: any) => vol.userId === session.user?.id)
    : false;
  const [enrolled, setEnrolled] = useState(hasJoined);
  const [spots, setSpots] = useState({
    filled: v.spotsFilled,
    total: v.spotsTotal,
  });
          ...
          {v.images && v.images.length > 0 && (
            <div className="flex gap-3 overflow-x-auto pb-2">
              {v.images.map((img: string, i: number) => (
                <div
                  key={i}
                  className="overflow-hidden rounded-2xl border border-white/5 shrink-0">
                  <img
                    src={img}
                    alt={`${v.title} ${i + 1}`}
          ...
          <div>
            ...
            <div className="mt-3 flex flex-wrap gap-4 text-sm text-slate-500">
              ...
              <span className="flex items-center gap-1.5">
                ...
                {spots.filled}/{spots.total} volunteers
              </span>
            </div>
          </div>
        ...
        <div className="space-y-6">
          <div className="glass-card p-6 space-y-3">
            {enrolled ? (
              ...
              <button
                onClick={handleJoin}
                disabled={joining || isFull || !v.isActive}
          ...
          <div className="glass-card p-6">
            ...
            <dl className="space-y-3 text-sm">
              <div className="flex justify-between">
                ...
                <dd className="text-slate-300">{v.category}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-slate-500">Date</dt>
                <dd className="text-slate-300">{formatDateTime(v.date)}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-slate-500">Spots</dt>
                <dd className="text-slate-300">
                  {spots.filled}/{spots.total}
                </dd>
              </div>
              <div className="flex justify-between">
                ...
                <dd
                  className={cn(
                    "font-medium",
                    v.isActive ? "text-emerald-400" : "text-slate-500",
                  )}>
