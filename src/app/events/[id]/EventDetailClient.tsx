...
import {
  ChevronLeft, CalendarDays, MapPin, Users, Clock, Loader2, Send,
  MessageSquare, Share2, UserPlus, CheckCircle2, Trash2,
} from "lucide-react";
import { formatDate, formatDateTime } from "@/lib/utils";

// eslint-disable-next-line @typescript-eslint/no-explicit-any
export default function EventDetailClient({ event, initialComments }: { event: any; initialComments: any[] }) {
  ...
  const handleComment = async () => {
    ...
    const result = await addComment({ targetType: "campaign", targetId: event._id, content: commentText });
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8 animate-fade-in">
      <Link href="/events" className="inline-flex items-center gap-1.5 text-sm text-slate-400 hover:text-white transition-colors mb-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          {event.image && (
            <div className="overflow-hidden rounded-2xl border border-white/5">
              <img src={event.image} alt={event.title} className="w-full max-h-[350px] object-cover" />
            </div>
          )}

          <div>
            {isPast && <span className="rounded-full bg-slate-700 px-3 py-1 text-xs font-medium text-slate-400 mb-3 inline-block">Past Event</span>}
            <h1 className="text-2xl font-bold text-white sm:text-3xl">{event.title}</h1>
            <div className="mt-3 flex flex-wrap gap-4 text-sm text-slate-500">
              <span className="flex items-center gap-1.5"><CalendarDays className="h-4 w-4" />{formatDate(event.date)}</span>
              {event.endDate && <span className="flex items-center gap-1.5"><Clock className="h-4 w-4" />Until {formatDate(event.endDate)}</span>}
        ...
        <div className="space-y-6">
          <div className="glass-card p-6 space-y-4">
            <div className="flex items-center gap-3">
              <button onClick={handleJoin} disabled={joining || joined || isPast}
                className={`flex-1 flex items-center justify-center gap-2 rounded-xl py-3 text-sm font-medium transition-all ${
                  joined ? "bg-emerald-500/20 text-emerald-400" : "bg-indigo-500/10 text-indigo-400 hover:bg-indigo-500/20"
                }`}>
                {joining ? <Loader2 className="h-4 w-4 animate-spin" /> : joined ? <CheckCircle2 className="h-4 w-4" /> : <UserPlus className="h-4 w-4" />}
          ...
          <div className="glass-card p-6">
            ...
            <dl className="space-y-3 text-sm">
              <div className="flex justify-between"><dt className="text-slate-500">Organizer</dt><dd className="text-slate-300">{event.organizer}</dd></div>
              <div className="flex justify-between"><dt className="text-slate-500">Attendees</dt><dd className="text-slate-300">{attendeeCount}{event.maxAttendees ? ` / ${event.maxAttendees}` : ""}</dd></div>
              <div className="flex justify-between"><dt className="text-slate-500">Date</dt><dd className="text-slate-300">{formatDateTime(event.date)}</dd></div>
              {event.endDate && <div className="flex justify-between"><dt className="text-slate-500">End Date</dt><dd className="text-slate-300">{formatDateTime(event.endDate)}</dd></div>}
            </dl>
          </div>
        </div>
      </div>
